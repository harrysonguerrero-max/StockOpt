...
SOURCES = [
    ("parts_master, demand_history, inventory_current",
...
def render_markdown() -> str:
    ...
    """
    lines = ["# Diccionario de datos - MVP SupplyOpt", "", INTRO, "", "## Fuentes",
    ...
    for name, spec in TABLES.items():
        lines += ["", "---", "",
                  f"## {name} - {spec['title']}", "", spec["summary"], "",
                  "| Columna | Tipo | Unidad | Origen | Descripcion |",
                  "|---|---|---|---|---|"]
        lines += [f"| {column} | {kind} | {unit} | {origin} | {description} |"
                  for column, kind, unit, origin, description in spec["columns"]]
